...
if(isset($_POST["submit"]) && $_POST["submit"]!="") {
	...
	for($i=0;$i<$usersCount;$i++) {
		mysqli_query($conn, "UPDATE multiple_update set userName='" . $_POST["userName"][$i] . "', password='" . $_POST["password"][$i] . "', firstName='" . $_POST["firstName"][$i] . "', lastName='" . $_POST["lastName"][$i] . "' WHERE userId='" . $_POST["userId"][$i] . "'");
	}
		...
		<div style="width:500px;">
			<table border="0" cellpadding="10" cellspacing="0" width="500" align="center">
				...
				for($i=0;$i<$rowCount;$i++) {
					$result = mysqli_query($conn, "SELECT * FROM multiple_update WHERE userId='" . $_POST["users"][$i] . "'");
